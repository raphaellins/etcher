...
const findDrive = (state, device) => {
  ...
  const maybeDrive = state.get('availableDrives').find((drive) => {
    return drive.get('device') === device
  })

  return _.invoke(maybeDrive, [ 'toJS' ])
...
const storeReducer = (state = DEFAULT_STATE, action) => {
  switch (action.type) {
    ...
    case ACTIONS.SELECT_DRIVE: {
      ...
      const selectionDevices = state.getIn([ 'selection', 'devices' ])

      return state.setIn([ 'selection', 'devices' ], selectionDevices.add(action.data))
    }
    ...
    case ACTIONS.UNSELECT_DRIVE: {
      ...
      return state.setIn([ 'selection', 'devices' ], selectionDevices.delete(action.data))
    }
